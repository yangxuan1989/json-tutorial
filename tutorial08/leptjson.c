...
void lept_copy(lept_value* dst, const lept_value* src) {
    ...
    switch (src->type) {
        ...
        case LEPT_ARRAY:
			...
			dst->type = src->type;
			for (j = 0; j < src->u.a.size; j++)
			{
				lept_copy(&dst->u.a.e[j], &src->u.a.e[j]);
			}
            break;
        case LEPT_OBJECT:
			lept_free(dst);
...
int lept_is_equal(const lept_value* lhs, const lept_value* rhs) {
    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT:
			...
			{
				const lept_value* temp = lept_find_object_value(rhs, lhs->u.o.m[i].k, lhs->u.o.m[i].klen);
				if (temp != NULL && !lept_is_equal(&lhs->u.o.m[i].v, temp))
					return 0;
			}
            return 1;
        default:
...
void lept_erase_array_element(lept_value* v, size_t index, size_t count) {
	...
	size_t i;
	size_t stop_pos = index + count;
	for (i = index; i < v->u.a.size; i++)
	{
		if (i < stop_pos)
			lept_free(&v->u.a.e[i]);
		if (i + count < v->u.a.size)
...
void lept_remove_object_value(lept_value* v, size_t index) {
	...
	if (index < v->u.o.size - 1)
		memmove(&v->u.o.m[index], &v->u.o.m[index + 1], (v->u.o.size - 1 - index) * sizeof(lept_member));
	v->u.o.size -= 1;
}
